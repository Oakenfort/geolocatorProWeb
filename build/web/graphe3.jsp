            ...
            var chart = new Chart(ctx, {
                ...
                data: {
                    labels: [<% for (Smartphone s : smartphoneService.findAll()) {%>"<%= s.getImei()%>",<% }%>""],
                            datasets: [{
                                    data: [<% for (Position p : positionService.findAll()) { for (Smartphone s : smartphoneService.findAll()) {%>
                                 <%= positionService.nbrPositionParSmartParMois(s.getImei(),new java.sql.Date(p.getDate().getTime()))%>
                                     ,<% }}%>0],
                                    ...
                                    label:"nombre des positions par smartphone par mois"
